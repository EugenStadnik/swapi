...
/**
 * @author ystadnik A place for common methods of all pages
 *
 */
public abstract class AbstractPage {
	...
	public void waitForElementContainsText(final By locator, long timeoutSeconds, long pollingTimeMillis) {
		...
		wait.until(new Function<WebDriver, Boolean>() {
			...
			public Boolean apply(WebDriver driver) {
				return driver.findElement(locator).getText().replaceAll("\n", " ").trim().matches(
						"^(?!.?\\<!--\\sngIf\\:\\sitem\\.caret\\s==\\strue\\s--\\>).+((?<=\\s\\<!--\\sngIf\\:\\sitem\\.caret\\s==\\strue\\s--\\>)|(?<!\\s\\<!--\\sngIf\\:\\sitem\\.caret\\s==\\strue\\s--\\>))$");
			}
		});
	}
	...
	public String getText(By by) {
		waitForLocator(by);
		return driver.findElement(by).getText().replaceAll("\n", " ").trim();
	}
	...
	public void waitToClickOnElement(By locator, long timeoutSeconds, long pollingTimeMillis) {
		given().ignoreExceptions().with().pollDelay(pollingTimeMillis, MILLISECONDS).and()
				.pollInterval(pollingTimeMillis, MILLISECONDS).await().atMost(timeoutSeconds, SECONDS)
